...
<template_setup>
**Before writing any video code, set up the project from the template.**

This skill includes a starter template at `template/` (relative to this SKILL.md). Copy the entire template directory into the user's working directory to bootstrap the video project:

1. Copy `template/` contents into the user's current working directory (or a subdirectory if the user specifies one)
2. Run `npm install` to install dependencies
3. Run `npm run dev` to start the dev server

The template includes:
- `useVideoPlayer` hook for scene management and recording
- Vite config with path aliases (`@/` maps to `client/src/`)
- Tailwind CSS setup with CSS variables
- Export script for rendering to mp4
- shadcn/ui component configuration

Do not skip this step. Every video project starts from the template.
</template_setup>
...
- Do not remove the `window.startRecording` or `window.stopRecording` calls. The hook calls `window.startRecording?.()` on mount and `window.stopRecording?.()` after the first complete pass -- the recording/export pipeline depends on both. Removing or skipping either causes exports to fail.
...
- Do not remove the `window.startRecording` or `window.stopRecording` calls. If you rewrite the video component or replace `useVideoPlayer`, keep `window.startRecording?.()` called on mount and `window.stopRecording?.()` called after the first full loop -- the export pipeline needs both.
</video_structure>
...
**`useVideoPlayer` hook** (from `@/lib/video`): Use `useVideoPlayer({ durations: SCENE_DURATIONS })`. Returns `{ currentScene }`. Handles recording API (`window.startRecording` and `window.stopRecording`), scene advancement, and looping automatically. Do not remove the `window.startRecording` or `window.stopRecording` calls -- see `<video_structure>` for details.
...
- Use `useVideoPlayer` from `@/lib/video` for scene management (see `<video_structure>` for the pattern). Do not remove the `window.startRecording` or `window.stopRecording` calls -- both are required for video export.
...
After building a new video from scratch, use the Agent tool to delegate a finalization pass:

- Launch a `general-purpose` subagent with the prompt: "Finalize video polish -- verify all content fits within the 16:9 frame without cutoff, remove any responsive breakpoints, and confirm the loop is clean. Follow the instructions in references/finalize_playback.md exactly."
- The agent should read and follow `references/finalize_playback.md` (relative to this skill directory)

After the agent completes, restart the dev server so the user sees the final result.
</completing_your_run>
